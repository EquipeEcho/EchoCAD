...
def processar_dxf(caminho_arquivo):
    ...
    for entidade in msp.query('LINE LWPOLYLINE'):
        ...
        if layer.startswith(config["eletrica"]["prefixo_layer"].upper()):
            bitola = extrair_valor_por_dicionario(layer, config["eletrica"]["bitolas_mapeadas"], 2.5)
            chave = f"fio_{bitola}mm2"
            
            if json_saida := entidade.dxftype() == 'LINE':
                comprimento = math.hypot(entidade.dxf.end.x - entidade.dxf.start.x, entidade.dxf.end.y - entidade.dxf.start.y)
            else:
                comprimento = sum(math.hypot(p2.x-p1.x, p2.y-p1.y) for p1, p2 in zip(entidade.vertices(), ezdxf.math.Vec3.generate(entidade.vertices()[1:])))
        ...
        # Processando Linhas de Canos Hidráulicos
            ...
            diametro = extrair_valor_por_dicionario(layer, config["hidraulica"]["diametros_mapeados"], 25.0)
        ...
        # Processando Linhas de Escada
            ...
            largura = 1.20
            match_larg = math.trunc(extrair_valor_por_dicionario(layer, {"LARG-1-20": 1.20, "LARG-0-90": 0.90}, 1.20))
            
            if entidade.dxftype() == 'LINE':
                comp_horizontal = math.hypot(entidade.dxf.end.x - entidade.dxf.start.x, entidade.dxf.end.y - entidade.dxf.start.y)
            else:
                comp_horizontal = sum(math.hypot(p2.x-p1.x, p2.y-p1.y) for p1, p2 in zip(entidade.vertices(), ezdxf.math.Vec3.generate(entidade.vertices()[1:])))
        ...
        # Processando Linhas de Paredes cadastrados no JSON
            ...
            for idx, seg in enumerate(segmentos):
                ...
                dados_paredes.append({
                    ...
                    "volume_bruto_m3": round(comp_seg * parede_config["altura"] * parede_config["espessura"], 3),
                    "descontos_aberturas": [],
        ...
        # Processando Linhas de Vigas cadastradas no JSON
            ...
            for idx, seg in enumerate(segmentos):
                ...
                dados_vigas.append({
                    ...
                    "volume_m3": round(comp_seg * viga_config["base_m"] * viga_config["altura_m"], 3),
                    "colunas_associadas": []
                })
    ...
    for bloco in msp.query('INSERT'):
        ...
        if viga_mapeada:
            ...
            dados_vigas.append({
                ...
                "volume_m3": round(comprimento_estimado * viga_mapeada["base_m"] * viga_mapeada["altura_m"], 3),
                "colunas_associadas": []
            })
        ...
        if coluna_mapeada:
            dados_colunas.append({
                ...
                "volume_m3": round(coluna_mapeada["largura_m"] * coluna_mapeada["profundidade_m"] * coluna_mapeada["altura_m"], 3)
            })
    ...
    for abertura in dados_aberturas:
        ...
        for parede in dados_paredes:
            distancia = calcular_distancia_ponto_linha(ponto_abertura, parede["inicio"], parede["fim"])
            
            if distancia <= TOLERANCIA:
                vol_desconto = round(abertura["largura"] * abertura["altura"] * parede["espessura_m"], 3)
    ...
    for coluna in dados_colunas:
        ...
        for viga in dados_vigas:
            if "inicio" not in viga or "fim" not in viga:
                continue
            distancia = calcular_distancia_ponto_linha(ponto_coluna, viga["inicio"], viga["fim"])
            if distancia <= TOLERANCIA:
    ...
    for parede in dados_paredes:
        total_descontos = sum(ab["volume_descontado_m3"] for ab in parede["descontos_aberturas"])
        parede["volume_liquido_m3"] = round(parede["volume_bruto_m3"] - total_descontos, 3)
        ...
        for ab in parede["descontos_aberturas"]:
            if ab["tipo"] == "porta": total_portas += 1
            if ab["tipo"] == "janela": total_janelas += 1
    ...
    return {
        ...
        "laje": dados_laje,
        "infraestrutura": {
            "eletrica": [{"tipo": k, "comprimento_m": round(v, 2)} for k, v in dados_eletrica.items()],
            "hidraulica": [{"tipo": k, "comprimento_m": round(v, 2)} for k, v in dados_encanamento.items()]
        },
        "escadas": processamento_escadas,
        "resumo_global": {
            "quantidade_total_portas": total_portas,
            "quantidade_total_janelas": total_janelas,
            "volume_total_descontado_vãos_m3": round(volume_total_descontado_aberturas, 3),
