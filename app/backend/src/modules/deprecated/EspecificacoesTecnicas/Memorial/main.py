    ...
    for reg in tabela_f:
        for i in range(max(1, len(reg.get("itens", [1])))):
            ...
            set_cell(
                ...
                r,
                5,
                reg.get("contra_incendio", {}).get("reservatorio", {}).get("tipo"),
            )
            set_cell(
                sheet,
                r,
                6,
                reg.get("contra_incendio", {}).get("reservatorio", {}).get("un"),
            )
            set_cell(sheet, r, 7, reg.get("contra_incendio", {}).get("registros"))
            set_cell(sheet, r, 8, reg.get("contra_incendio", {}).get("valvulas"))
            set_cell(
                sheet, r, 9, reg.get("contra_incendio", {}).get("dutos", {}).get("tipo")
            )
            set_cell(
                sheet, r, 10, reg.get("contra_incendio", {}).get("dutos", {}).get("m")
            )
            ...
            set_cell(
                ...
                r,
                12,
                reg.get("pressurizadas", {}).get("reservatorio", {}).get("tipo"),
            )
            set_cell(
                sheet,
                r,
                13,
                reg.get("pressurizadas", {}).get("reservatorio", {}).get("un"),
            )
            set_cell(sheet, r, 14, reg.get("pressurizadas", {}).get("registros"))
            set_cell(sheet, r, 15, reg.get("pressurizadas", {}).get("valvulas"))
            set_cell(
                sheet, r, 16, reg.get("pressurizadas", {}).get("dutos", {}).get("tipo")
            )
            set_cell(
                sheet, r, 17, reg.get("pressurizadas", {}).get("dutos", {}).get("m")
            )
...
def main():
    ...
    if caminho_template is None:
        print(
            "Arquivo de template 'model_memorial.xlsx' não encontrado nas pastas esperadas."
        )
    ...
    backup_path = caminho_template.with_name(
        caminho_template.stem + "_backup" + caminho_template.suffix
    )
    try:
    ...
    try:
        ...
        alt_path = caminho_template.with_name(
            caminho_template.stem + "_out" + caminho_template.suffix
        )
        try:
