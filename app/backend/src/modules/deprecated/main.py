...
def run_extraction(
    prompt_usuario: str, dxf_path: str, project_id: int, gerar_excel: bool = True
):
    ...
    agente_surveyor = create_surveyor_agent(
        [dxf.get_grouped_entities_summary, dxf.get_detailed_entities]
    )

    # 2. Criação do Team
    team = create_team(
        dxf, [agente_contexto, agente_layer_select, agente_espacial, agente_surveyor]
    )
    ...
    resultado = {"json": response.content, "arquivo_excel": None}

    # 4. Geração do Excel (opcional)
    if gerar_excel:
        try:
            resultado_excel = _gerar_excel_a_partir_json(
                response.content, prompt_usuario, project_id
            )
...
def _gerar_excel_a_partir_json(
    json_resultado: str, prompt_usuario: str, project_id: int
) -> str:
    """
    ...
    if any(word in prompt_lower for word in ["parede", "alvenaria", "alv", "arq"]):
        ...
        word in prompt_lower for word in ["elétric", "elet", "fio", "circuito", "luz"]
    ):
        disciplina = "eletrica"
    elif any(
        word in prompt_lower for word in ["hidráulic", "hid", "água", "cano", "esgoto"]
    ):
    ...
    if hasattr(populator.wb, "active"):
        ...
        ws_info.cell(
            row=2, column=1
        ).value = f"Data: {datetime.now().strftime('%d/%m/%Y %H:%M:%S')}"
